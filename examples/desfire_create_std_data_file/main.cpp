/**
 * @file main.cpp
 * @brief DESFire CreateStdDataFile example
 *
 * Flow:
 *   1) Select target application
 *   2) Authenticate with an application key
 *   3) Create standard data file
 */
...
namespace
{
    ...
    void printUsage(const char* exeName)
    {
        ...
        std::cout << "  --file-size <n>                          Required (1..16777215)\n";
    ...
    Args parseArgs(int argc, char* argv[])
    {
        ...
        for (int i = 2; i < argc; ++i)
        {
            ...
            else if (opt == "--file-size")
            {
                args.fileSize = parseUInt32(requireValue("--file-size"));
        ...
        if (!fileSizeProvided)
        {
            throw std::runtime_error("--file-size is required");
        }
        if (args.fileSize == 0U || args.fileSize > 0x00FFFFFFU)
        {
            throw std::runtime_error("--file-size must be in range 1..16777215");
        }
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        auto createResult = desfire->createStdDataFile(
            args.fileNo,
        ...
        if (!createResult)
        {
            ...
            {
                std::cout << "CreateStdDataFile returned DuplicateError; continuing (--allow-existing)\n";
                return 0;
            }

            std::cerr << "CreateStdDataFile failed: " << createResult.error().toString().c_str() << "\n";
            return 1;
        }

        std::cout << "CreateStdDataFile OK\n";
        return 0;
    }
