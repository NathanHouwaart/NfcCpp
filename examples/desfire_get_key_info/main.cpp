/**
 * @file main.cpp
 * @brief DESFire GetKeySettings + GetKeyVersion example
 *
 * Flow:
 *   1) Select application (default PICC 000000)
 *   2) Optional authenticate
 *   3) GetKeySettings
 *   4) GetKeyVersion for selected key number
 */
...
namespace
{
    ...
    const char* keyTypeFlagsToText(uint8_t keyTypeFlags)
    {
        switch (keyTypeFlags)
        {
            case 0x00:
    ...
    void printUsage(const char* exeName)
    {
        ...
        std::cout << "  --authenticate                    Run Authenticate before key queries\n";
        std::cout << "  --auth-mode <legacy|iso|aes|des|2k3des|3k3des> Default: iso\n";
        std::cout << "  --auth-key-no <n>                 Default: 0\n";
        std::cout << "  --auth-key-hex <hex>              Required with --authenticate\n";
        std::cout << "  --query-key-no <n>                Key number for GetKeyVersion, default: 0\n";
    }
    ...
    Args parseArgs(int argc, char* argv[])
    {
        ...
        for (int i = 2; i < argc; ++i)
        {
            ...
            else if (opt == "--query-key-no")
            {
                args.queryKeyNo = parseByte(requireValue("--query-key-no"));
            }
            else
            {
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        std::cout << "DESFire GetKeySettings + GetKeyVersion Example\n";
        std::cout << "COM: " << args.comPort << " @ " << args.baudRate << "\n";
        std::cout << "AID: " << toHex(args.aid) << "\n";
        std::cout << "Query key no: " << static_cast<int>(args.queryKeyNo) << "\n";
        ...
        auto keySettingsResult = desfire->getKeySettings();
        if (!keySettingsResult)
        {
            std::cerr << "GetKeySettings failed: " << keySettingsResult.error().toString().c_str() << "\n";
            return 1;
        }

        const auto keySettings = keySettingsResult.value();
        const uint8_t keySettings1 = keySettings[0];
        const uint8_t keySettings2 = keySettings[1];
        const uint8_t keyCount = static_cast<uint8_t>(keySettings2 & 0x0F);
        const uint8_t keyTypeFlags = static_cast<uint8_t>(keySettings2 & 0xC0);

        std::cout << "GetKeySettings OK\n";
        std::cout << "  KeySettings1: 0x" << std::hex << std::uppercase << std::setw(2) << std::setfill('0')
                  << static_cast<int>(keySettings1) << std::dec << "\n";
        std::cout << "  KeySettings2: 0x" << std::hex << std::uppercase << std::setw(2) << std::setfill('0')
                  << static_cast<int>(keySettings2) << std::dec << "\n";
        std::cout << "  KeyCount: " << static_cast<int>(keyCount) << "\n";
        std::cout << "  KeyType: " << keyTypeFlagsToText(keyTypeFlags) << "\n";

        auto keyVersionResult = desfire->getKeyVersion(args.queryKeyNo);
        if (!keyVersionResult)
        {
            std::cerr << "GetKeyVersion failed: " << keyVersionResult.error().toString().c_str() << "\n";
            return 1;
        }

        const uint8_t keyVersion = keyVersionResult.value();
        std::cout << "GetKeyVersion OK\n";
        std::cout << "  Key[" << static_cast<int>(args.queryKeyNo) << "] version: 0x"
                  << std::hex << std::uppercase << std::setw(2) << std::setfill('0')
                  << static_cast<int>(keyVersion) << std::dec << "\n";
