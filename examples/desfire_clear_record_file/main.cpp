/**
 * @file main.cpp
 * @brief DESFire ClearRecordFile example
 *
 * Flow:
...
namespace
{
    struct Args
    {
        ...
        bool commitAfterClear = false;
        bool allowNoChanges = false;
    ...
    void printUsage(const char* exeName)
    {
        ...
        std::cout << "  --authenticate                            Authenticate before ClearRecordFile\n";
        ...
        std::cout << "  --commit                                  Run CommitTransaction after clear\n";
        std::cout << "  --allow-no-changes                        Treat Commit NoChanges as success\n";
    ...
    Args parseArgs(int argc, char* argv[])
    {
        ...
        for (int i = 2; i < argc; ++i)
        {
            ...
            {
                args.commitAfterClear = true;
            }
            else if (opt == "--allow-no-changes")
            {
                args.allowNoChanges = true;
            }
        ...
        if (args.allowNoChanges && !args.commitAfterClear)
        {
            throw std::runtime_error("--allow-no-changes can only be used together with --commit");
        }
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        std::cout << "DESFire ClearRecordFile Example\n";
        ...
        auto clearResult = desfire->clearRecordFile(args.fileNo);
        if (!clearResult)
        {
            if (args.allowMissing &&
                clearResult.error().is<error::DesfireError>() &&
                clearResult.error().get<error::DesfireError>() == error::DesfireError::FileNotFound)
            {
                std::cout << "ClearRecordFile returned FileNotFound; treating as success (--allow-missing)\n";
                return 0;
            }

            std::cerr << "ClearRecordFile failed: " << clearResult.error().toString().c_str() << "\n";
            return 1;
        }

        std::cout << "ClearRecordFile OK\n";

        if (args.commitAfterClear)
        {
            auto commitResult = desfire->commitTransaction();
            if (!commitResult)
            {
                if (args.allowNoChanges &&
                    commitResult.error().is<error::DesfireError>() &&
                    commitResult.error().get<error::DesfireError>() == error::DesfireError::NoChanges)
                {
                    std::cout << "CommitTransaction returned NoChanges; treating as success (--allow-no-changes)\n";
                    return 0;
                }
        ...
        {
            std::cout << "Note: ClearRecordFile is transactional; run again with --commit to persist.\n";
        }
