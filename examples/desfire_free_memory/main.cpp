...
namespace
{
    ...
    void printUsage(const char* exeName)
    {
        ...
        std::cout << "  --authenticate                            Authenticate before FreeMemory\n";
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        std::cout << "DESFire FreeMemory Example\n";
        ...
        auto freeMemoryResult = desfire->freeMemory();
        if (!freeMemoryResult)
        {
            std::cerr << "FreeMemory failed: " << freeMemoryResult.error().toString().c_str() << "\n";
            return 1;
        }

        const uint32_t freeBytes = freeMemoryResult.value();
        const double freeKiB = static_cast<double>(freeBytes) / 1024.0;
        std::cout << "Free memory: " << freeBytes << " bytes (" << std::fixed << std::setprecision(2)
                  << freeKiB << " KiB)\n";
