/**
 * @file main.cpp
 * @brief DESFire GetFileIDs example
 *
 * Flow:
 *   1) Select application
 *   2) Optional authenticate
 *   3) Execute GetFileIDs
 */
...
namespace
{
    ...
    void printUsage(const char* exeName)
    {
        ...
        std::cout << "  --authenticate                    Authenticate before file ID query\n";
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        std::cout << "DESFire GetFileIDs Example\n";
        ...
        auto fileIdsResult = desfire->getFileIds();
        if (!fileIdsResult)
        {
            std::cerr << "GetFileIDs failed: " << fileIdsResult.error().toString().c_str() << "\n";
            return 1;
        }

        const auto& fileIds = fileIdsResult.value();
        std::cout << "File count: " << fileIds.size() << "\n";
        if (fileIds.empty())
        {
            std::cout << "No files in selected application.\n";
        }
        else
        {
            std::cout << "File IDs:";
            for (size_t i = 0; i < fileIds.size(); ++i)
            {
                std::cout << ' ' << std::hex << std::uppercase << std::setw(2) << std::setfill('0')
                          << static_cast<int>(fileIds[i]) << std::dec;
            }
            std::cout << "\n";
        }
