...
```powershell
.\examples\desfire_create_backup_data_file\build_example.ps1
```

Batch:

```bat
examples\desfire_create_backup_data_file\build_example.bat
```

## Command Syntax

```powershell
.\build\examples\Debug\desfire_create_backup_data_file_example.exe <COM_PORT> [options]
```
...
| `--allow-existing` | No | off | Continue when `CreateBackupDataFile` returns `DuplicateError`. |
...
- `CreateBackupDataFile` is an application-level command. Select and authenticate that application first.
- Most apps require master-key authentication for file creation.
- Backup data files use transaction semantics for writes: data written with `WriteData` is finalized on `CommitTransaction` and discarded on `AbortTransaction`.
- This example only creates the file. Transaction commands for backup writes are separate commands.
- Use `--allow-existing` for idempotent reruns while iterating.

## ChangeKeySettings Context

`ChangeKeySettings` does not create files. It changes `KeySettings1` policy bits for the selected PICC/application:

- whether master key/config can be changed,
- whether listing/create/delete can be done without master-key auth,
- which key is required for key changes.

So use:

- `ChangeKeySettings` to control security policy.
- `CreateBackupDataFile` to create a transactional data file.
