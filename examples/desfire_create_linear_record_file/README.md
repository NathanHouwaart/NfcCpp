...
```powershell
.\examples\desfire_create_linear_record_file\build_example.ps1
```

Batch:

```bat
examples\desfire_create_linear_record_file\build_example.bat
```

## Command Syntax

```powershell
.\build\examples\Debug\desfire_create_linear_record_file_example.exe <COM_PORT> [options]
```

## Command Key and Purpose

- DESFire command key (INS): `0xC1`
- Command name: `CreateLinearRecordFile`
- Purpose: create a linear record file where records are appended until `max-records` is reached.

Linear record files are useful for append-only logs such as events, counters, or transaction entries.
...
| `--allow-existing` | No | off | Continue when `CreateLinearRecordFile` returns `DuplicateError`. |
...
- `CreateLinearRecordFile` is an application-level command. Select and authenticate that application first.
