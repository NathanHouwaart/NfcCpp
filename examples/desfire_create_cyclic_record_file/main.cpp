/**
 * @file main.cpp
 * @brief DESFire CreateCyclicRecordFile example
 *
 * Flow:
 *   1) Select target application
 *   2) Authenticate with an application key
 *   3) Create cyclic record file
 */
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        std::cout << "DESFire CreateCyclicRecordFile Example\n";
        ...
        auto createResult = desfire->createCyclicRecordFile(
            args.fileNo,
        ...
        if (!createResult)
        {
            ...
            {
                std::cout << "CreateCyclicRecordFile returned DuplicateError; continuing (--allow-existing)\n";
                return 0;
            }

            std::cerr << "CreateCyclicRecordFile failed: " << createResult.error().toString().c_str() << "\n";
            return 1;
        }

        std::cout << "CreateCyclicRecordFile OK\n";
        return 0;
    }
