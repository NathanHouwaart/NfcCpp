/**
 * @file main.cpp
 * @brief DESFire GetCardUID example
 *
 * Flow:
 *   1) Select PICC (000000)
 *   2) Authenticate with PICC key
 *   3) Execute GetCardUID
 */
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        std::cout << "DESFire GetCardUID Example\n";
        ...
        auto uidResult = desfire->getRealCardUid();
        if (!uidResult)
        {
            std::cerr << "GetCardUID failed: " << uidResult.error().toString().c_str() << "\n";
            return 1;
        }

        const auto& uid = uidResult.value();
        std::cout << "Card UID (" << uid.size() << " bytes): " << toHex(uid) << "\n";
        return 0;
    }
