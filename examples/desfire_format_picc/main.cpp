/**
 * @file main.cpp
 * @brief DESFire FormatPICC example
 *
 * Flow:
 *   1) Select application (typically PICC: 000000)
 *   2) Optional authenticate
 *   3) Execute FormatPICC
 */
...
namespace
{
    ...
    void printUsage(const char* exeName)
    {
        ...
        std::cout << "  --authenticate                            Authenticate before FormatPICC\n";
        ...
        std::cout << "  --confirm                                 Required to execute destructive format\n";
    }
    ...
    Args parseArgs(int argc, char* argv[])
    {
        ...
        if (!args.confirm)
        {
            throw std::runtime_error("--confirm is required for FormatPICC");
        }
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        std::cout << "DESFire FormatPICC Example\n";
        std::cout << "COM: " << args.comPort << " @ " << args.baudRate << "\n";
        std::cout << "AID: " << toHex(args.aid) << "\n";
        std::cout << "WARNING: This operation erases all applications/files on the PICC.\n";
        ...
        if (args.authenticate)
        {
            const etl::vector<uint8_t, 24> authKey = toEtl<24U>(args.authKey);
        ...
        auto formatResult = desfire->formatPicc();
        if (!formatResult)
        {
            std::cerr << "FormatPICC failed: " << formatResult.error().toString().c_str() << "\n";
            return 1;
        }

        std::cout << "FormatPICC OK\n";
        return 0;
    }
